function main()
    ...
    figure;
    x = imread('./binary_image.jpg');
    m = imread('./pic6/binary_image.png"');
    subplot(1,2,1);
    imshow(x);
    title('Cover Image');
    subplot(1,2,2);
    imshow(m);
    title('Message Image');
    saveas(gcf, './pic6/LSB_img/origional.png');
...
function lsb_watermarked = Hide(cover, message)
    ...
    saveas(gcf, './pic6/LSB_img/lsb_watermarked.png');
end
...
function Extract(lsb_watermarked)
    ...
    saveas(gcf, './pic6/LSB_img/extracted.png');
end